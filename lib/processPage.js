...
function processCSS(file, siteStorage) {
  ...
  for (let min in minifiers.minifiers) {
    ...
    promise = promise.then(() => {
      ...
      if (data.bug) {
        siteStorage.bugs[file].push({ minifier: min, error: data.bug });
      } else {
        return fs.writeFileSync(path.join(siteStorage.paths.css, min + '_' + file), data.minified);
      }
    }).catch((err) => {siteStorage.bugs[file].push({ minifier: min, error: data.bug }); return err;});
  }
  return promise;
}

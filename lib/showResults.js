...
$buildSitePanel = ($, parent, site, report, min) => {
  $(_minifierPanel(site, panelCounter)).appendTo(parent.find('.panel-body'));
}
...
let $buildMinifierPanel = ($, storage, min) => {
  ...
  if (bugcount > 0) {
    output = $(_minifierPanel(`<a data-toggle="collapse" href="#panel-collapse-${panelCounter}">${min} - Failed!</a>`, panelCounter));
    ...
    for (let site in storage.sites) {
      ...
      $buildSitePanel($, output, site, report, min)
    }
  } else {

let cheerio    = require('cheerio'),
    path       = require('path'),
    open       = require("open"),
    getMinInfo = require('./minifiers').getMinifierInfo,
      ...
      <div class="container-fluid">
        ...
        <div class="navbar-right">
          <ul class="nav navbar-nav social-links">
            ...
            <li>
              <a class="github-button" href="https://github.com/Deeech/css-minificator-tests" data-icon="octicon-star" data-style="mega" aria-label="Star Deeech/css-minificator-tests on GitHub">Star</a>
            </li>
            <li>
              <a class="github-button" href="https://github.com/Deeech/css-minificator-tests/issues" data-icon="octicon-issue-opened" data-style="mega" aria-label="Issue Deeech/css-minificator-tests on GitHub">Issue</a>
            </li>
          </ul>
        </div>
      </div><!-- /.container-fluid -->
...
let $buildMinifierPanel = ($, min) => {
  ...
  if (bugCount > 0 || isFailed) {
    ...
    for (let site in storage.sites) {
      ...
      if (bugcount > 0 || diff) siteErrorsCounter++;
    }

    $minifierPanel = $(_minifierPanel(`<a data-toggle="collapse" href="#panel-collapse-${panelCounter}">${min} ${getMinInfo(min).version} - ${siteErrorsCounter}/${Object.keys(storage.sites).length} Failed!${isFailed}</a>`, panelCounter));
    $minifierPanel.addClass('panel-danger');

  } else {
    $minifierPanel = $(_minifierPanel(`<a data-toggle="collapse" href="#panel-collapse-${panelCounter}">${min} ${getMinInfo(min).version} - Success!</a>`, panelCounter));
